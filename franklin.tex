...
\begin{proof}
  ...
  is distributed like $\max\{A_1,\ldots,A_R\}$ where where $R$ is a
  $\mathrm{geometric}(p)$ random variable and each $A_i$ is an independent
...
Q_t(L)\rangle$ is certainly not iid.  Each mountain produces exactly one
peak (in $F_t(L)$), while each lake may produce zero or more seamounts.

The following lemma quantifies the probability that a mountain peak
($i$) is immediately followed by another mountain peak $\cw_{Q_t(L)}(i)$.
...
\begin{proof}
   The event we are studying occurs when $\hat{F}_t(L)$ contains no
   elements in the interval $[i,\cw_{F_t(L)}(i)]$.  Now, we know that
   there is at least one element $j$ in this interval with $X_j < t$.
   Therefore $G_{<t}$ contains a lake $C$ with vertices $v_1,\ldots,v_m$
   such that $i<v_1<\cdots<v_m<\cw_{F_t(L)}(i)$. The component $C$
   does not contribute any element to $\hat{F}_t(L)$ if and only if the
   sequence $X_{v_1},\ldots,X_{v_m}$ has no local maxima except possibly
   $X_{v_1}$ and $X_{v_m}$.
   ...
   Now, the size of the lake $C$ (i.e., the value of
   $m$) has a geometric distribution given by
   $\Pr\{|C|=m\}=(1-p)^{m-1}p$.  Letting $\mathcal{E}_C$ denote the event
   ``$C$  contributes at least one elements to $\hat{F}_t(L)$'', we have
   \[
...
can derive a closed form for the probability in \lemref{empty-lake}:
\[
q = 
...
\begin{lem}\lemlabel{lower-bound-ii}
  ...
  $\Pr\{i\in F_t(Q_t(L))\}= (1/2)(2p-p^{2}(1+e^{2-2p}))$ and the sequence 
...
Optimizing over $p$, we find that the probability
in \lemref{lower-bound-ii} this function has a global
maximum at $p=-(1/2)W(-2/e^2)$, where $W$ is the Lambert
$W$-function\footnote{$W(z)=x$ where $x$ is the unique solution to $xe^x
= z$.} For this value of $p$, 
\[
c_1 = -\frac{1}{8} \, {\left(e^{\operatorname{W}(-2/e^2) + 2} + 1\right)} \operatorname{W}(-2/e^2)^{2} - \frac{1}{2} \operatorname{W}(-2/e^2)
>
0.0809512797364893
\]

\begin{thm}
   For any $k\in\N$, $p_{k} \ge \sqrt{c_1}^k > 0.284519384^k$.
\end{thm}
