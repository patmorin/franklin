...
%        {3c^3 + 6ac + 6bc + 8ab + 4a^2 + 4c^2b + 4a^2 + 4b^2 + 4c^2a} \\
     & = \frac
        { 4c^3/3 + 3ab + 3ac + 3bc + 2a^2 + 2b^2 + 2c^2a + 2c^2b 
        - x_{ca} - x_{bc} - x_{a^2} - x_{b^2} - x_{c^2a} - x_{c^2b}}
     ...
     & = \frac{1}{2}- 
        ...
        {3c^3 + 8ab + 6ac + 6bc + 4a^2 + 4b^2 + 4c^2a + 4c^2b} \nonumber \\
     & = \frac{1}{2}- 
        \frac{c^3/6 + ab}{3q_3 + 4q_4} \nonumber \\
     & \le \frac{1}{2}- 
        \frac{c^3/6 + ab}{4q_3 + 4q_4} \nonumber \\
     & = \frac{1}{2}- 
        \frac{c^3/6 + ab}{4} \nonumber
\end{align}

Thus, we have a situation in which $\alpha$ can be very close $1/2$
if and only if $ab$ is very small.  Next we show that 

Again, we can study two consecutive rounds.  Recall that, by definition,
with probability $a^2-x_{a^2}$ a group is an $a^2$-group that generates a
block of size 2.  The key observation is that, by symmetry, the resulting
block is equally likely to an $a$-block or a $b$-block.
Similarly, with probability $b^2-x_{b^2}$ a group is a $b^2$-group that generates a block of size 2 that is equally likely to be an $a$-block or $b$-block.
...
Letting $a'$, $b'$ and $c'=1-a'-b'=q_1$ denote the probabilities of $a$-, $b$-, and $c$-blocks in $R(B)$ we have
\[
   a',b' \ge (1/2)(a^2-x_{a^2}+b^2-x_{b^2}+c^3/3)  \enspace .
\]  
and, in particular, the product $a'b' \ge ((1/2)(a^2-x_{a^2}+b^2-x_{b^2}+c^3/3))^2$.
...
\begin{align}
    \alpha' & = \frac{q'_1+2q'_2}{3p_3 + 4p_4} \\
            ...
            & = \left(\frac{1}{2}-\frac{q_1^3/6 + a'b'}{4}\right)
               \left(\frac{q_1+2q_2}{3p_3+4p_4} \right) \\
            & \le \left(\frac{1}{2}-\frac{q_1^3/6 + (1/2)(a^2-x_{a^2}+b^2-x_{b^2}+c^3/3)}{4}\right)
               \left(\frac{q_1+2q_2}{3p_3+4p_4} \right) \eqlabel{ap} \\
\end{align}
This last expression is a function of 8 variables $a$, $b$, $x_{a^2}$,
$x_{b^2}$, $x_{ca}$, $x_{bc}$, $x_{c^2a}$, $x_{c^2b}$.  It is clear
from \eqref{clear} that \eqref{ap} is maximized when $x_{ca} = x_{bc}
=x_{c^2a}= x_{c^2b}=0$.
